...
public class Pack200Task extends Pack
{
	    ...
	    throws BuildException
	{
		if (p200ConfigFile != null
			...
			throw new BuildException("Pack200 property file attribute must "
			        + "exist and not represent a directory!", getLocation());
		}
		if (doGZIP) {
	...
	protected void pack()
	{
		 ...
		 * this should be using some kind of ant logging service anyway
		String statusStr = doRepack ? "Repack with Pack200"
		        : "Packing with Pack200";
		System.out.println(statusStr + " " + source);
		...
		try {
			...
			if (doRepack) {
				...
				if (p200ConfigFile != null) {
					...
					pc_is.close();
					for (Map.Entry me : pFile.entrySet()) {
						unpkr.properties().put((String) me.getKey(),
						        (String) me.getValue());
					}
				}
